/** 背诵逻辑 — 中考必背 · 六十篇（works 来自 recite-data-zhongkao.js） */
...
function loadProgress() {
  try { return JSON.parse(localStorage.getItem('exam60_progress')) || {}; }
  catch(e) { return {}; }
}
function saveProgress(p) { localStorage.setItem('exam60_progress', JSON.stringify(p)); }
...
function loadNotes() {
  try { return JSON.parse(localStorage.getItem('exam60_notes')) || {}; }
  catch(e) { return {}; }
}
function saveNote(id, text) {
  const n = loadNotes();
  n[id] = text;
  localStorage.setItem('exam60_notes', JSON.stringify(n));
}

function loadTheme() { return window.cjgResolveTheme ? cjgResolveTheme('exam60_theme','') : (localStorage.getItem('exam60_theme') !== null ? localStorage.getItem('exam60_theme') : (/Android|iPhone|iPad|iPod|Mobile|HarmonyOS|HUAWEI|HONOR/i.test(navigator.userAgent) ? 'dark' : '')); }
function saveTheme(t) { localStorage.setItem('exam60_theme', t); }
...
function renderWorkList() {
  ...
  filtered.forEach(w => {
    ...
    html += '<div class="work-item' + active + '" onclick="navigateToWork(' + w.id + ')">' +
      ...
      (hintLabel ? '<span class="wi-hint" title="数字编码：' + w.id + ' → ' + hint + '">' + hintLabel + '</span>' : '') +
      (isRead ? '<span class="wi-badge" title="L3 默写已达标">✓</span>' : '') +
      '</div>';
  });
  list.innerHTML = html;

  document.getElementById('totalProgressBar').style.width = Math.round(totalRead / 60 * 100) + '%';
  document.getElementById('totalProgressText').textContent = Math.round(totalRead / 60 * 100) + '%';
}
...
function navigateToWork(id) {
  ...
  localStorage.setItem('exam60_work', id);
  renderContent();
  renderWorkList();
  closeSidebar();
...
function switchMode(mode) {
  ...
  localStorage.setItem('exam60_mode', currentMode);
  localStorage.setItem('exam60_reciteLevel', reciteLevel);
  renderContent();
}
...
function renderContent() {
  ...
  let html = '<div class="work-container">' +
    ...
    '<div class="wh-title">' + w.id + '. ' + w.title +
      (hintWord ? ' <span class="wh-hint-badge" title="数字编码 #' + w.id + '">' + emoji + ' ' + hintWord + '</span>' : '') +
    '</div>' +
...
function renderReciteL3(text) {
  ...
  const w = getWork();
  const wubiHref = '8655.html?corpus=junior#' + currentWork;
...
(function init() {
  ...
  if (urlMode) {
    ...
    localStorage.setItem('exam60_mode', currentMode);
    localStorage.setItem('exam60_reciteLevel', String(reciteLevel));
  } else {
    const savedMode = localStorage.getItem('exam60_mode');
    if (savedMode) { currentMode = savedMode; reciteLevel = parseInt(localStorage.getItem('exam60_reciteLevel')) || 1; }
  }
  ...
  const savedWork = parseInt(localStorage.getItem('exam60_work'));
  const startWork = (hash >= 1 && hash <= 60) ? hash : (savedWork >= 1 && savedWork <= 60) ? savedWork : 1;
